...
connection.getConnection();
  ...
  this.getBlocks = function(cback) {
    ...
    async.each(
      _this.stats.pools,
      function(pool, pcb) {
        if (_this.stats.pools[pool.name].confirmed && _this.stats.pools[pool.name].apiBlocks.blocks) {
          _this.stats.pools[pool.name].apiBlocks.blocks.forEach(block => {
            allBlocks.push(block);
          });
        }

        // console.log('this stats pool', _this.stats.pools);
        ...
        //     allBlocks[pool.name + '-' + _this.stats.pools[pool.name].pending.blocks[i].split(':')[2]] =
        ...
        //     allBlocks[pool.name + '-' + _this.stats.pools[pool.name].confirmed.blocks[i].split(':')[2]] =
        //       _this.stats.pools[pool.name].confirmed.blocks[i];

        pcb();
        //console.log('HI PORTAL STATS', JSON.stringify(_this.stats));
      },
      function(err) {
        // cback(_this.portalStats);
        cback(allBlocks);
      },
    );
  };
  ...
  this.getCurrentStatus = function(cback) {
    connection.execute(
     `SELECT
        height
...
//	  console.log(last_mined_block);
    ...
    getNodeHeight();
    cback({"last_mined_height": last_mined_block, "node_height": node_height}); 
  ...
  this.getGlobalStats = function(callback) {
    ...
    async.each(
      ...
      function(client, callback) {
        ...
        client.client.multi(redisCommands).exec(async function(err, replies) {
          if (err) {
	...
	//      console.log("Repl 1 + 6", replies[i + 7]);
              ...
              var coinStats = {
                ...
                poolStats: {
                  ...
                  validBlocks: blockStats[0].validBlocks ? blockStats[0].validBlocks || 0 : 0, //replies[i + 2] ? replies[i + 2].validBlocks || 0 : 0,
                  invalidShares: replies[i + 2] ? replies[i + 2].invalidShares || 0 : 0,
                  totalPaid: replies[i + 2] ? replies[i + 2].totalPaid || 0 : 0,
                  networkBlocks: node_height, //replies[i + 2] ? replies[i + 2].networkBlocks || 0 : 0,
                  networkSols: replies[i + 2] ? replies[i + 2].networkSols || 0 : 0,
                  networkSolsString: getReadableNetworkHashRateString(
                    replies[i + 2] ? replies[i + 2].networkSols || 0 : 0,
                  ),
                ...
                // TODO REPLACE THIS WITH SQL CODE
                blocks: {
                  pending: blocksStats['pending'], //replies[i + 3],
                  confirmed: blocksStats['confirmed'], //replies[i + 4],
                  orphaned: blocksStats['orphaned'], //replies[i + 5],
                },
      ...
      function(err) {
        ...
        Object.keys(allCoinStats).forEach(function(coin) {
          ...
          var shareMultiplier = 510000;//Math.pow(2, 32) / algos[coinStats.algorithm].multiplier;
          ...
          for (var worker in coinStats.workers) {
            var _workerRate =
              (shareMultiplier * coinStats.workers[worker].shares) / portalConfig.website.stats.hashrateWindow;
            var _wHashRate = (_workerRate / 1000000) * 2;
            coinStats.workers[worker].luckDays = (
              ((_networkHashRate / _wHashRate) * _blocktime) /
              (24 * 60 * 60)
            ).toFixed(3);
            coinStats.workers[worker].luckHours = (((_networkHashRate / _wHashRate) * _blocktime) / (60 * 60)).toFixed(
              3,
            );
          ...
          for (var miner in coinStats.miners) {
            var _workerRate =
              (shareMultiplier * coinStats.miners[miner].shares) / portalConfig.website.stats.hashrateWindow;
            var _wHashRate = (_workerRate / 1000000) * 2;
        ...
        // console.log('PORTAL STATS SAVING TO STATS', portalStats);
        ...
        redisStats
          ...
          .exec(function(err, replies) {
            if (err) logger.error(logSystem, 'Historics', 'Error adding stats to historics ' + JSON.stringify(err));
          });
        callback();
      },
    );
