...
module.exports = function(logger, poolConfig){
    ...
    const query = (...args) => {
     return new Promise((resolve, reject) => {
      mysqlPool.query(args[0], args[1], function(err, result) {
        if (err) reject(err);
        resolve(result);
      });
    });
    ...
    this.handleShare = function(isValidShare, isValidBlock, shareData) {
        ...
        //console.log(`************** HANDLE SHARE PROCESSOR  ${JSON.stringify(shareData)} *******`);
    ...
    try {
     ...
     // }
      ...
      console.log('Insert error when adding share: ' + JSON.stringify(err));
    }
