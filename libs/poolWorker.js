...
module.exports = function(logger) {
    ...
    switch (message.type) {
      ...
      case 'coinswitch':
        ...
        logger.debug(
          ...
          logComponent,
          logSubCat,
          'Proxy message for ' + algo + ' from ' + oldCoin + ' to ' + newCoin,
        );
          ...
          redisClient.hset('proxyState', algo, newCoin, function(error, obj) {
            if (error) {
              logger.error(
                ...
                logComponent,
                logSubCat,
                'Redis error writing proxy config: ' + JSON.stringify(err),
              );
            } else {
  ...
  Object.keys(poolConfigs).forEach(function(coin) {
    ...
    if (poolOptions.mposMode && poolOptions.mposMode.enabled) {
      ...
      handlers.auth = function(ip, port, api_key, authCallback) {
        mposCompat.handleAuth(ip, port, api_key, authCallback);
      };

      handlers.share = function(isValidShare, isValidBlock, data) {
        if (isValidShare) mposCompat.handleShare(isValidShare, isValidBlock, data);
      };

      handlers.block = function(blockData) {
        mposCompat.handleBlock(blockData);
      };

      handlers.blockUpdate = function(reply) {
          mposCompat.handleBlockUpdate(reply);
      };

      handlers.diff = function(workerName, diff, poolWorkerId) {
        mposCompat.handleDifficultyUpdate(workerName, diff, poolWorkerId);
      };

      handlers.disconnect = function(poolWorkerId) {
        mposCompat.handleDisconnect(poolWorkerId);
      };
    ...
    var authorizeFN = function(ip, port, api_key, callback) {
      handlers.auth(ip, port, api_key, function(authorized, results) {
        var authString = authorized ? 'Authorized' : 'Unauthorized ';

        logger.debug(logSystem, logComponent, logSubCat, authString + ' [' + ip + ']');
        callback({
          error: null,
          authorized: authorized,
          disconnect: false,
          poolWorkerId: results.poolWorkerId,
          poolUserId: results.poolUserId,
          poolCoinId: results.poolCoinId,
          userSetDifficulty: results.userSetDifficulty,
        });
      });
    };
    ...
    pool
      .on('block', function(blockData) {
        handlers.block(blockData);
       }).on('blockupdate', function(reply) {
        handlers.blockUpdate(reply);
      })
      .on('share', function(isValidShare, isValidBlock, data, poolUserId, poolCoinId) {
      ...
      .on('difficultyUpdate', function(workerName, diff, poolWorkerId) {
        logger.debug(
          logSystem,
          logComponent,
          logSubCat,
        ...
        handlers.diff(workerName, diff, poolWorkerId);
      })
      .on('minerDisconnect', function(poolWorkerId) {
        logger.debug(logSystem, logComponent, logSubCat, 'Miner disconnected: ' + poolWorkerId);
        handlers.disconnect(poolWorkerId);
      })
        ...
        Object.keys(proxySwitch[switchName].ports).forEach(function(port) {
          var f = net
            .createServer(function(socket) {
              ...
              if (pools[currentPool]) pools[currentPool].getStratumServer().handleNewClient(socket);
              else pools[initalPool].getStratumServer().handleNewClient(socket);
            })
    ...
    Object.keys(portalConfig.switching).forEach(function(switchName) {
      ...
      for (var port in portalConfig.switching[switchName].ports) {
        ...
        if (portalConfig.switching[switchName].ports[port].diff) {
          if (!pool.options.ports.hasOwnProperty(port)) pool.options.ports[port] = {};
